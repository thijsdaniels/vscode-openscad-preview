...
export class NumberFieldInputEvent extends CustomEvent<{ value: number }> {
  static readonly eventName = "input" as const;
  constructor(value: number) {
    super(NumberFieldInputEvent.eventName, { detail: { value }, bubbles: true, composed: true });
  }
}

export class NumberFieldChangeEvent extends CustomEvent<{ value: number }> {
  static readonly eventName = "change" as const;
  constructor(value: number) {
    super(NumberFieldChangeEvent.eventName, { detail: { value }, bubbles: true, composed: true });
  }
}

declare global {
  interface HTMLElementTagNameMap {
    "scad-number-field": ScadNumberField;
  }
}

@customElement("scad-number-field")
export class ScadNumberField extends LitElement {
  ...
  private handlePointerMove = (e: PointerEvent) => {
    ...
    if (newValue !== this.value) {
      this.value = newValue;
      this.dispatchEvent(new NumberFieldInputEvent(this.value));
    }
  };
  ...
  private fireChangeEvent() {
    this.dispatchEvent(new NumberFieldChangeEvent(this.value));
  }
    ...
    return html`
      <div class="container" @pointerdown=${this.handlePointerDown}>
        ${this.min !== undefined && this.max !== undefined
          ...
          : nothing}
        <span class="value-text">${this.value}${this.suffix}</span>
        ${this.isEditing
          ? html`
              <input
                type="number"
